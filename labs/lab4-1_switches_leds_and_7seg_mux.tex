...
\end{circuitikz}
\caption{Schematic for buttons (\texttt{SW1-SW4}) and LEDs (\texttt{LED1-LED5}).
The multiplexed-display driver (segment fan-out, digit-select transistors) is
shown separately in Fig.~2, for both display polarities.}
\end{figure}
...
\node[draw, fill=accentlight, minimum width=3.0cm, minimum height=1.5cm, align=center]
     at (2.8,5)   (dig1) {\textbf{DIGIT1}\\segment \texttt{a} (shown)};
\node[draw, fill=accentlight, minimum width=3.0cm, minimum height=1.5cm, align=center]
     at (2.8,1.2) (dig2) {\textbf{DIGIT2}\\segment \texttt{a} (shown)};
...
\draw (Q1.B) -- ++(-1.1,0) to[R, l=1k$\Omega$] ++(-1.5,0) node[left]{\texttt{PB4}};
\draw (Q2.B) -- ++(-1.1,0) to[R, l=1k$\Omega$] ++(-1.5,0) node[left]{\texttt{PB5}};

\end{circuitikz}
\caption{\textbf{Common cathode} (this lab's build): NPN low-side switch sinks
\texttt{CC1}/\texttt{CC2} to GND -- digit is selected by driving \texttt{PB4}/\texttt{PB5}
\textbf{HIGH}.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\textwidth}
...
\node[pnp, rotate=180] at (6,3.1)  (Q1) {};
\node[pnp, rotate=180] at (6,-0.7) (Q2) {};

\draw (dig1.south) |- (Q1.C) node[pos=0.9, above right]{CA1};
\draw (dig2.south) |- (Q2.C) node[pos=0.9, above right]{CA2};

\draw (Q1.E) -- ++(0,-0.8) coordinate(vcc1);
\node[vcc] at (vcc1) {};
\node[right=0.02cm of vcc1] {\texttt{+5V}};
\draw (Q2.E) -- ++(0,-0.8) coordinate(vcc2);
\node[vcc] at (vcc2) {};
\node[right=0.02cm of vcc2] {\texttt{+5V}};

\draw (Q1.B) -- ++(1.1,0) to[R, l=1k$\Omega$] ++(1.5,0) node[right]{\texttt{PB4}};
\draw (Q2.B) -- ++(1.1,0) to[R, l=1k$\Omega$] ++(1.5,0) node[right]{\texttt{PB5}};

\end{circuitikz}
\caption{\textbf{Common anode} (alternative): PNP high-side switch feeds
\texttt{+5V} into \texttt{CA1}/\texttt{CA2} -- digit is selected by driving
\texttt{PB4}/\texttt{PB5} \textbf{LOW} (inverted vs.\ common cathode), and each
segment now lights when its MCU pin is driven LOW instead of HIGH.}
\end{subfigure}
\caption{Digit-driver comparison for the two 7-segment display polarities, segment
\texttt{a} shown representative of \texttt{a-g} -- segments \texttt{b-g} repeat the
identical pattern (own 330~$\Omega$ resistor from \texttt{PC1-PC5}/\texttt{PD2}, tied
to both digits' matching pin) and are omitted here only to keep the figure readable.
Only the common-node connection and transistor type change between (a) and (b) --
the button/LED wiring (Fig.~1) and the segment fan-out resistor network are
identical either way. See the \emph{Write It} exercise below for the matching
segment-code and select-logic changes needed in firmware.}
\end{figure}
