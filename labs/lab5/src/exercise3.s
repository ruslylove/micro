...
; Lab 5 - In-Lab Exercise 3: Input phase (set first and second)
;
; Builds the full 4-phase multiplexed REFRESH loop (display + keypad scan
; interleaved on the shared PB2-PB5 pins) and wires up only the R1/R2 keys
; from the Function Key Map:
;   S1 inc first   S2 dec first   S3 inc second   S4 dec second
;   S5 inc inc first   S6 dec dec first   S7 inc inc second   S8 dec dec second
; R3/R4 (shift/add/subtract) are left as no-ops -- that's Exercise 4.
;
; Register usage:
;   tmp        (r16) segment/port scratch
;   nib        (r17) hex_to_seg digit argument / general scratch
...
refresh:
    ; ---- Phase 1: first's high nibble, row 1 (key_index 0-3) ----
    mov nib, first
    swap nib
    andi nib, 0x0F
    rcall hex_to_seg
...
p1_end:
    ...
    ; ---- Phase 2: first's low nibble, row 2 (key_index 4-7) ----
    mov nib, first
    andi nib, 0x0F
    rcall hex_to_seg
    sbi DDRB, 3
...
p2_end:
    ...
    ; ---- Phase 3: second's high nibble, row 3 (key_index 8-11, inactive) ----
    mov nib, second
    swap nib
    andi nib, 0x0F
    rcall hex_to_seg
...
p3_end:
    ...
    ; ---- Phase 4: second's low nibble, row 4 (key_index 12-15, inactive) ----
    mov nib, second
    andi nib, 0x0F
    rcall hex_to_seg
    sbi DDRB, 5
...
; In: held_key = key_index (0-15). Keys 8-15 are still inactive (Exercise 4).
dispatch_key:
    cpi held_key, 0x00
    breq dk_s1
    cpi held_key, 0x01
